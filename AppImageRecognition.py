...
def process_images(target_img):
    ...
    # Feature detection (Handle errors if SIFT is not available)
    try:
        sift = cv2.SIFT_create()
    except AttributeError:
        st.error("SIFT feature detection is not available in this environment.")
        return

    target_kp, target_des = sift.detectAndCompute(target_img_gray, None)
    if target_des is None:
        st.error("Could not detect features in the uploaded image.")
        return
...
def extract_row_number(filename, df_length):
    """Extracts row number from filename, ensuring it's valid."""
    try:
